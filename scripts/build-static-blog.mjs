...
function renderArticlePage(entry, config, siteSettings) {
  ...
  const jsonLdPayload = {
    ...
    dateModified: entry._updatedAt || entry.publishedAt || undefined,
...
async function getEntries(config) {
  const buildEntriesQuery = (postTypes, kinds) => {
    ...
    return [
      ...
      "        questions[]{questionText, allowMultiple, options[]{label, points}},",
      "        results[]{minScore, maxScore, title, description, ctaText, ctaWhatsApp, icon}",
      "      }",
      "    }",
      "  }",
      "}"
